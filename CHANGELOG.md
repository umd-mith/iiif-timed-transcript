...
- 903b888: Element host surface — five playback events for analytics (`iiif-player-play`, `iiif-player-pause`, `iiif-player-ended`, `iiif-player-seeked`, `iiif-player-rate-change`); a `label` attribute that sets the player's accessible name; `::part()` handles for host styling (`controls`, `button`, `speed`, `progress`, `transcript`, and each segment); CSS custom states `:state(playing)`, `:state(loading)`, `:state(error)`; and a `crossorigin` attribute passed through to the media element. The package now ships `dist/element/custom-elements.json` (a Custom Elements Manifest), guarded by a build-time drift check against the element's real attributes and typed event map. The README gains an "Embedding in a CMS" section. (#63)
...
- dcb4fa6: Element packaging — `package.json` gains a `sideEffects` field (listing only the IIFE element file) so bundlers can drop the unused ESM element, and the non-functional `./element/iife` export is removed (it resolved a classic script as a module; vendor `dist/element/iiif-transcript-player.iife.js` directly). A JavaScript property set on the element before it upgrades now wins over a same-named attribute, instead of being silently blocked — the path every CMS page takes when markup parses before the element's script loads. Public export `getLocale` is removed (nothing consumed it). None of these had shipped in a release. (#60)
...
- 903b888: **Element events are renamed** to an `iiif-player-` prefix — `playerrefavailable` → `iiif-player-ready`, `playererror` → `iiif-player-error`, `canvaschange` → `iiif-player-canvas-change`; the old names are gone (safe because the element has never shipped in a release, so the entries above are superseded by these names). Element styles are hoisted into constructed stylesheets, so the player renders under a strict Content Security Policy that forbids inline `<style>`. A manifest pointing at IIIF Auth-gated media now reports the error with `source: "auth"`. (#63)
