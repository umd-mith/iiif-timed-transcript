...
- 0ad964a: Accessibility — new `IIIFPlayer.Captions` toggle (placed in the element's control bar), a transcript auto-scroll pause control, and an always-mounted status live region that announces transcript load and failure (WCAG 4.1.3, Status Messages). Forced-colors mode (Windows High Contrast) is supported (`dd63be1`), and new focus and segment-indicator color tokens convey interface state without relying on color alone (WCAG 1.4.1, Use of Color). `SyncController.setAutoScrollEnabled` is exposed for hosts. (#62)
