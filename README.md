...
| `onmatchactivate` | `(annotation: Annotation, index: number) => void` | from context           | Explicit activation (Enter, or "Go to match" — shown when `Transcript`'s `searchSeekBehavior` is `"activate"`). Seeks once, regardless of `searchSeekBehavior`. |
