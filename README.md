...
Captions discovered from the manifest are attached as `<track>` elements exactly as before. When the transcript panel is populated — from `annotations="auto"`, from a `Transcript` `annotations` prop, or from Root's — `Viewer` switches every attached track to `mode = "hidden"` **once per canvas**, so the same text is not shown twice. The tracks stay attached, so a viewer can turn them back on — and keep them on — two ways. Passing `controls` shows the browser's native video controls, whose caption menu selects the track; `Viewer` defaults to `controls={false}`, so that menu is absent by default. Alternatively, render the library's `IIIFPlayer.Captions` button, which toggles captions with no native controls needed. An explicit `tracks` prop is left alone.
...
| `searchSeekBehavior`       | `"change" \| "activate"`                                                  | `"change"`           | `"change"` preserves legacy behavior — every selected search match seeks immediately. `"activate"` seeks only on explicit activation (Enter or "Go to match"); typing and previous/next only update highlights. **Recommended: `"activate"`.**                                                                                |
| `scrollToSeek`             | `boolean`                                                                 | `true`               | When `false`, transcript scrolling never seeks; a deliberate scroll instead enters Browsing (reading mode) so the next playback update can't pull the panel back. Independent of `readingMode`. Changeable at runtime. **Recommended: `false`.**                                                                              |
| `readingMode`              | `boolean` (bindable)                                                      | `false`              | `false` is Following (playback may scroll the panel, and may seek if `scrollToSeek` is on); `true` is Browsing (the panel is detached — playback still updates the active-passage highlight, but never scrolls or seeks). Turning it back off scrolls the passage at the current time into view without seeking. Panel-local. |
...
**Recommended configuration: `searchSeekBehavior="activate"` and `scrollToSeek={false}`.** Playback follows the transcript until the reader searches, scrolls, or explicitly enables reading mode — search and browsing no longer fight playback for control of the panel. With no new props, `Transcript` keeps today's exact behavior (search-driven and scroll-driven seeking).

Whenever `children` are present, `Transcript` renders one "Follow along" switch (`role="switch"`, `aria-checked`, `data-following`) — turning it off enters Browsing — and, only while Browsing, a "Jump to current" button that returns to Following by scrolling the passage at the media's current time into view (never seeking). Both are bare: no appearance, styled by the host (the default `<iiif-transcript-player>` element ships a styled pair — see "Use without Svelte" below). This single control replaces the former auto-scroll-pause button; there is no separate "pause auto-scroll but keep scroll-to-seek" state.
...
| `onmatchchange`   | `(matches: Annotation[], index: number) => void`  | from context           | Debounced match-list/selection change callback                                                                                                                                    |
| `onmatchactivate` | `(annotation: Annotation, index: number) => void` | from context           | Explicit activation (Enter, or "Go to match" — shown when `Transcript`'s `searchSeekBehavior` is `"activate"` or while Browsing). Seeks once, regardless of `searchSeekBehavior`. |
| `onmatchnavigate` | `(annotation: Annotation, index: number) => void` | from context           | Fired only on previous/next navigation (never typing or activation). The default scrolls the newly-selected match into view through the context; never seeks.                     |
| `onqueryinput`    | `(value: string) => void`                         | from context           | Synchronous input-intent callback, fired before the debounce — the default enters Browsing on a nonempty query in `"activate"` mode.                                              |
| `class`           | `string`                                          | `""`                   | CSS class                                                                                                                                                                         |
...
| `search-seek-behavior` | `searchSeekBehavior` | `string`  | `"change"`             | `"change"` or `"activate"` — see `Transcript`'s prop above. Any other value is a non-fatal host `iiif-player-error`, once per bad value per connection, and falls back to `"change"`. **Recommended: `"activate"`.**                                                                                                                                                                                                                                                                                                                              |
| `scroll-to-seek`       | `scrollToSeek`       | `boolean` | `true`                 | Value-based, not presence-based: `"false"` (case-insensitive) is `false`; `"true"`, the empty string, and a bare `scroll-to-seek` attribute are `true`; a removed attribute is the default, `true`, silently. Any other string value is a non-fatal host error and falls back to `true`. A property write accepts a real boolean directly. **Recommended: `false`.**                                                                                                                                                                              |
| `reading-mode`         | `readingMode`        | `boolean` | `false`                | Same value-based parsing as `scroll-to-seek` (default `false`). **Reactive and reflected**, like `canvas-index`: the element's own "Follow along"/"Jump to current" controls, and any property write, write this attribute back through a guarded update that cannot loop. Exposes `:state(browsing)` on the host while `true` (alongside the existing `:state(playing)`/`:state(loading)`/`:state(error)`), for CSS like `iiif-transcript-player:state(browsing) { ... }`.                                                                       |
...
`search-seek-behavior`, `scroll-to-seek`, and `reading-mode` work through markup, property writes, and property writes made before the element upgrades, same as every other attribute above. `scroll-to-seek` and `reading-mode` are the element's own manual value-based owner rather than the generic Boolean-attribute parsing `autoplay` uses — Svelte's built-in `type: "Boolean"` is presence-based and cannot tell `reading-mode="false"` from `reading-mode="true"`, so this library parses and reflects the two by hand.

**Recommended: `search-seek-behavior="activate"` and `scroll-to-seek="false"`.**

```html
<iiif-transcript-player
  manifest-url="https://example.org/manifest.json"
  search-seek-behavior="activate"
  scroll-to-seek="false"
></iiif-transcript-player>
```

The default element renders a styled "Follow along" switch and "Jump to current" button in its shadow DOM (unlike the bare `Transcript` component's unstyled pair) and reflects `reading-mode` as the reader toggles it, so `iiif-transcript-player:state(browsing) { ... }` and `iiif-transcript-player[reading-mode] { ... }` both work from page CSS.
