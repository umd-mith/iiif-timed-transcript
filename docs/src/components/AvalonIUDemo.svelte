<script lang="ts">
  /**
   * Demo: a production Avalon manifest from Indiana University's Media
   * Collections Online, played with annotations="auto".
   *
   * Everything interesting happens in the library: getPrimaryResource
   * resolves the Choice-wrapped HLS quality ladder, hls.js attaches for the
   * application/x-mpegURL stream, and the transcript is built from the
   * canvas's supplementing WebVTT annotation. The host writes none of it.
   */
...
<div class="w-full avalon-iu-demo">
...
<style>
  :global(.avalon-iu-demo [data-annotation-id]) {
    @apply p-3 mb-2 rounded transition-colors text-left;
  }

  :global(.avalon-iu-demo [data-annotation-id]:hover) {
    @apply bg-gray-100;
  }

  :global(.avalon-iu-demo [data-annotation-id][data-state="active"]) {
    @apply bg-blue-100 border-l-4 border-terracotta-500;
  }

  :global(.avalon-iu-demo [data-annotation-id][data-highlighted="true"]) {
    @apply bg-yellow-100;
  }

  :global(.avalon-iu-demo [data-annotation-id][data-current-match="true"]) {
    @apply bg-yellow-200 border-l-4 border-yellow-500;
  }

  :global(.avalon-iu-demo [data-annotation-id] .timestamp) {
    @apply text-xs text-gray-600 font-semibold block mb-1;
  }

  :global(.avalon-iu-demo [data-annotation-id] .text) {
    @apply text-gray-800 leading-relaxed;
  }

  /* transcript-panel must be the scroll container for SyncController */
  :global(.avalon-iu-demo .transcript-panel) {
    @apply flex-1 flex flex-col overflow-y-auto;
  }

  :global(.avalon-iu-demo .segments-container) {
    @apply p-4;
  }

  :global(.avalon-iu-demo .loading-message) {
    @apply p-4 text-sm text-gray-600;
  }

  :global(.avalon-iu-demo input[type="search"]) {
