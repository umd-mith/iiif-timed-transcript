...
<style>
  ...
  /* Reading mode: the "Follow along" switch and "Jump to current" button
     ship bare (semantics + state only) — this is the demo's own skin. */
  :global(.iiif-transcript-demo .follow-along-switch) {
  ...
  :global(.iiif-transcript-demo .follow-along-switch[aria-checked="true"]) {
    @apply bg-terracotta-500 border-terracotta-500 text-white;
  }

  :global(.iiif-transcript-demo .jump-to-current) {
