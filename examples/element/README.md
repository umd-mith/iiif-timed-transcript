...
What to look for: captions visible on the Cookbook video until the transcript populates (then hidden, toggleable via the CC button in the control bar); all eight duplicate-id segments present in the second player on plain.html; no "No transcript available." banner on Avalon; `iiif-player-error` with `source: "transcript"` only if a VTT fails; Bootstrap's button reset does not restyle the player controls.
