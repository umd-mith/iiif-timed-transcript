...
import { isHlsUrl, isHlsNativelySupported, createHlsAdapter, type HlsAdapter } from './hlsUtils';
...
describe('isHlsUrl', () => {
	...
	test('detects HLS via application/vnd.apple.mpegurl format', () => {
		expect(isHlsUrl('https://example.com/stream', 'application/vnd.apple.mpegurl')).toBe(true);
	});

	test('detects HLS via application/x-mpegURL format', () => {
		expect(isHlsUrl('https://example.com/stream', 'application/x-mpegURL')).toBe(true);
	});
...
describe('createHlsAdapter', () => {
	...
	test('attach loads source and attaches to media element', () => {
		...
		expect(instance.loadSource).toHaveBeenCalledWith('https://example.com/stream.m3u8');
	...
	test('attach replaces previous instance on re-attach', () => {
		...
		(Ctor as any).Events = { MANIFEST_PARSED: 'hlsManifestParsed', ERROR: 'hlsError' };
		...
		expect(instance2.loadSource).toHaveBeenCalledWith('https://example.com/stream2.m3u8');
	});
