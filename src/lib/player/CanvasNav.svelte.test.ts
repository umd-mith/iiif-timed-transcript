...
const twoCanvases: CanvasInfo[] = [
	{ index: 0, id: 'https://example.com/canvas/1', label: 'Part 1', duration: 90, mediaType: 'audio' },
	{ index: 1, id: 'https://example.com/canvas/2', label: 'Part 2', duration: 120, mediaType: 'video' }
];
...
describe('CanvasNav', () => {
	...
	test('renders canvas buttons from context', async () => {
		const { default: CanvasNav } = await import('./CanvasNav.svelte');
		const ctx = createMockPlayerContext({ canvasCount: 2, canvases: twoCanvases });
	...
	test('displays canvas label and duration', async () => {
		...
		expect(firstButton.querySelector('.canvas-label')?.textContent).toBe('Part 1');
		expect(firstButton.querySelector('.canvas-duration')?.textContent).toContain('1:30');

		const secondButton = target.querySelector('[data-canvas-index="1"]')!;
		expect(secondButton.querySelector('.canvas-label')?.textContent).toBe('Part 2');
		expect(secondButton.querySelector('.canvas-duration')?.textContent).toContain('2:00');
	});

	test('marks active canvas with data-state="active"', async () => {
		const { default: CanvasNav } = await import('./CanvasNav.svelte');
		const ctx = createMockPlayerContext({ canvasIndex: 1, canvasCount: 2, canvases: twoCanvases });
	...
	test('clicking canvas calls switchCanvas', async () => {
		...
		const secondButton = target.querySelector('[data-canvas-index="1"]') as HTMLElement;
		secondButton.click();
	...
	test('omits duration span when canvas has no duration', async () => {
		...
		const ctx = createMockPlayerContext({ canvasCount: 2, canvases: noDurationCanvases });
