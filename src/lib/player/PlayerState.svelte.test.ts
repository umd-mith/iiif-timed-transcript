...
describe('PlayerStateManager', () => {
	...
	describe('actions.seekTo', () => {
		test('warns and returns when media not ready', () => {
			...
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Cannot seek'));
		...
		test('warns on NaN time', () => {
			...
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid seek time'));
			warnSpy.mockRestore();
		});
	...
	describe('actions.setPlaybackRate', () => {
		...
		test('warns when no media element', () => {
			...
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Cannot set playback rate'));
			warnSpy.mockRestore();
		});
	});
