...
export function mergeIntoParagraphs(
  ...
  function flush() {
    if (current.length > 0) {
      paragraphs.push(
        buildParagraph(paragraphs.length, current, opts.speakers),
      );
      current = [];
      wordCount = 0;
    }
...
function shouldBreak(
  ...
  if (nextSegment) {
    ...
    if (
      opts.speakers &&
      hasSpeakerChange(opts.speakers, segment.id, nextSegment.id)
    ) {
      return true;
    }
  ...
  return (
    (wordCount >= opts.target && isSentenceEnd(segment.text)) ||
    wordCount >= opts.max
  );
}
