...
import Panel from "../../lib/transcript/Panel.svelte";
import type { Annotation, IIIFMediaViewerRef } from "../../lib/sync/types";
...
describe("Transcript.Panel - Structure & Props", () => {
  ...
  it("renders all annotations", () => {
    mount(Panel, {
      target,
      props: { annotations: mockAnnotations, viewer: mockViewer },
    });
    flushSync();
...
describe("Transcript.Panel - SyncController Integration", () => {
  ...
  it("calls viewer.seekTo when segment clicked", () => {
    ...
    const segment1 = target.querySelector(
...
describe("Transcript.Panel - Fullscreen Mode", () => {
  ...
  it("does not show fullscreen toggle by default", () => {
    ...
    const fullscreenButton = target.querySelector(
...
describe("Transcript.Panel - onSegmentClick callback", () => {
  ...
  it("suppresses seek when preventDefault is called", () => {
    const onSegmentClick = vi.fn(
      (_annotation: Annotation, event: { preventDefault: () => void }) => {
