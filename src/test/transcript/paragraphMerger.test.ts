...
import { mergeIntoParagraphs } from "../../lib/transcript/paragraphMerger";
  ...
  describe("empty and trivial inputs", () => {
    ...
    test("returns one paragraph for a single segment", () => {
      const segments = makeSegments([
        "Welcome to the Lakeland Digital Archive.",
      ]);
  ...
  describe("word-count threshold triggers paragraph break", () => {
    test("breaks into multiple paragraphs when word count exceeds target", () => {
      const segments = makeSegments(
        Array.from(
          { length: 25 },
          (_, i) =>
            `This is segment number ${i + 1} with some extra words to fill.`,
        ),
      );
      const result = mergeIntoParagraphs(segments);
    ...
    test("custom targetWordCount is respected", () => {
      const segments = makeSegments(
        Array.from(
          { length: 20 },
          (_, i) => `Segment ${i + 1} has about six words.`,
        ),
      );
      ...
      const segments: Annotation[] = [
        {
          ...
          startTime: 0,
          endTime: 10,
          text: "First thought about the community.",
        },
        {
          id: "seg-2",
          startTime: 15,
          endTime: 25,
          text: "Different topic about the school.",
        },
      ];
    ...
    test("does not break when gap is below threshold", () => {
      const segments: Annotation[] = [
        ...
        },
        {
          ...
          startTime: 11,
          endTime: 20,
          text: "Continuation of the same thought.",
        },
      ];
      const result = mergeIntoParagraphs(segments, { gapThreshold: 2.0 });
    ...
    test("handles very long transcript (400+ segments)", () => {
      const segments = makeSegments(
        Array.from(
          { length: 400 },
          (_, i) =>
            `This is segment number ${i + 1} from a long oral history recording.`,
        ),
      );
      const result = mergeIntoParagraphs(segments);
